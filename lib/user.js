 ...
 * @returns {Promise<GetUserFollowedMangaFeedResponse | ErrorResponse>} A promise that resolves to a {@link GetUserFollowedMangaFeedResponse} object
 */
exports.getUserFollowedMangaFeed = function (token, options) {
    if (token === undefined) {
        console.error('ERROR - getUserFollowedMangaFeed: Parameter `token` cannot be undefined');
        return;
    } else if (!('session' in token)) {
        console.error('ERROR - getUserFollowedMangaFeed: Parameter `token` missing required property `session`');
        return;
    }
