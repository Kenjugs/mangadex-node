 ...
 * 
 * @param {*} [options] A request options object to parse
...
exports.buildQueryStringFromOptions = function (options) {
    ...
    for (const key of Object.keys(options)) {
        ...
        } else if (options[key] instanceof Date && !isNaN(options[key])) {
            /** @type {Date} */
            const d = options[key];
...
exports.createHttpsRequestPromise = function (method, path, options) {
    ...
    if (options) {
        Object.assign(httpReqOptions, options);
    }

    /** @type {Promise<T | ErrorResponse>} */
    return new Promise((resolve, reject) => {
        const req = https.request(httpReqOptions, res => {
            const chunks = [];
            res.on('data', chunk => {
